...
//fake main
	...
	if (flag)
	{
		...
		colors = vertexColoring(max[0], colorSize, colors, row, graph);
		if (validate(colors))
		{
	...
	{
		cout << "Cannot Color this Graph with the Given Number of Colors Without Traversing Graph" << endl;
	}
...
vector<int> vertexColoring(int vertex, int colorSize, vector<int> , int vertexSize, vector< vector<int>>graph)
{
	...
	for (int colour = 1; colour <= colorSize; colour++)
	{
		...
		if ((count++  < vertexSize))
		{
			...
			{
				vertex = vertex % (vertexSize - 1) - 1;

			}

			//cout << "Rec" << vertex + 1 << " "<<endl;
...
bool isColor(int vertex,vector<int>colour,int size,vector< vector<int>>graph,int color)
{
	...
	{
		if ( ( graph[vertex ][i] !=0  && color == colour[i] ) )
		{
